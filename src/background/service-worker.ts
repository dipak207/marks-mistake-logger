...
type StatsStore = Record<string, QuestionStats>;
...
function cleanForSheet(value: unknown): string {
  return String(value ?? '')
    .replace(/<br\s*\/?>/gi, '\n')
    .replace(/\r\n/g, '\n')
    .trim();
...
function formatOptions(question: MarksQuestion): string {
  if (question.type !== 'singleCorrect') return '';
  return (question.options || [])
    .map((option, index) => `${optionLabel(index)}. ${cleanForSheet(option.text || option.image || '')}`)
    .join('\n');
}

function formatMarkedAnswer(question: MarksQuestion, attempt: AttemptPayload): string {
  if (question.type === 'numerical') return cleanForSheet(attempt.inputValue);
  ...
  if (index < 0) return cleanForSheet(markedId);
  const option = (question.options || [])[index];
  return `${optionLabel(index)}. ${cleanForSheet(option.text || option.image || '')}`;
}

function formatCorrectAnswer(question: MarksQuestion): string {
  if (question.type === 'numerical') return cleanForSheet(question.correctValue);
...
function accuracy(stats: QuestionStats): string {
...
function averageTime(stats: QuestionStats): string {
  if (!stats.timedAttempts) return '';
  const value = stats.totalTime / stats.timedAttempts;
  return `${Number(value.toFixed(2))} sec`;
}

function toSheetRow(stats: QuestionStats): (string | number)[] {
  return [
    stats.date,
    stats.questionUrl,
    stats.questionText,
...
async function readStore(): Promise<StatsStore> {
  ...
  return questionStats as StatsStore;
}
...
async function token(interactive: boolean): Promise<string> {
  ...
  if (!accessToken) throw new Error('Google authorization was not granted.');
  return accessToken;
}

async function googleFetch(url: string, init: RequestInit = {}, interactive = false, retry = true): Promise<Response> {
...
async function updateValues(spreadsheetId: string, range: string, values: unknown[][]): Promise<void> {
  ...
  if (!response.ok) throw new Error(`Sheets update failed: ${response.status}`);
}
...
async function findQuestionRow(spreadsheetId: string, questionId: string): Promise<number | null> {
  ...
  for (let index = 0; index < values.length; index += 1) {
    if (String(values[index]?.[0] ?? '') === questionId) return index + 2;
  }
  return null;
}

async function ensureSpreadsheet(interactive = false): Promise<{ spreadsheetId: string; spreadsheetUrl: string }> {
  ...
  if (stored.spreadsheetId) {
    return {
      ...
      spreadsheetUrl:
        stored.spreadsheetUrl || `https://docs.google.com/spreadsheets/d/${stored.spreadsheetId}/edit`
    };
  }
  ...
  if (!response.ok) throw new Error(`Spreadsheet creation failed: ${response.status}`);
...
async function upsertStatsRow(stats: QuestionStats): Promise<void> {
  if (!stats.wasEverWrong) return;
...
async function safeSync(stats: QuestionStats): Promise<void> {
  try {
...
async function recordAttempt(message: AttemptMessage): Promise<{ needsReason: boolean }> {
  ...
  const stats: QuestionStats = previous || {
    questionId,
    attempts: 0,
    wrong: 0,
    totalTime: 0,
  ...
  if (message.subject) stats.subject = message.subject;
  if (message.chapter) stats.chapter = message.chapter;
  ...
  if (stats.wasEverWrong) void safeSync(stats);

  return { needsReason: message.result === 'incorrect' };
}

async function setReason(questionId: string, reason: string): Promise<void> {
  const store = await readStore();
  const stats = store[questionId];
  if (!stats || !stats.wasEverWrong) return;
...
async function flushMistakes(): Promise<void> {
  const store = await readStore();
  for (const stats of Object.values(store)) {
    if (stats.wasEverWrong) await safeSync(stats);
  }
}

async function popupState(): Promise<Record<string, unknown>> {
  const store = await readStore();
  ...
  (async () => {
    switch (message?.type) {
      case 'ATTEMPT_RECORDED':
        sendResponse(await recordAttempt(message.payload as AttemptMessage));
        break;
      case 'SET_REASON':
