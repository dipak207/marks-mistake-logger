(() => {
  ...
  function looksLikeAttempt(value: unknown): value is JsonObject {
  ...
  function emitAttempt(value: JsonObject): void {
    emit('ATTEMPT_PAYLOAD', {
      ...
      optionsMarked: Array.isArray(value.optionsMarked)
        ? value.optionsMarked.filter((item): item is string => typeof item === 'string')
        : [],
  ...
  function inspectRequestBody(body: unknown): void {
    ...
    if (looksLikeAttempt(parsed)) emitAttempt(parsed);
  }

  function collectLabels(root: unknown): Record<string, string> {
  ...
  function inspectJsonResponse(data: unknown): void {
    ...
    if (Object.keys(labels).length) emit('ENTITY_LABELS', labels);
  }
  ...
  XMLHttpRequest.prototype.open = function (
    ...
    (this as XMLHttpRequest & { __mmlUrl?: string }).__mmlUrl = String(url);
      ...
      () => {
        try {
          ...
          if (this.responseType === 'json') {
            ...
            if (text && (text.startsWith('{') || text.startsWith('['))) data = JSON.parse(text);
          }
          if (data) inspectJsonResponse(data);
        } catch {
